      ...
      <div className="flex items-center justify-between pt-1">
        ...
        {isHighScore && lead.status !== 'CONVERTED' && (
          <Button
            ...
            className="font-mono text-[10px] tracking-widest bg-white text-black hover:bg-white/90 border-0 h-7 px-3 rounded-sm uppercase font-bold"
          >
...
export default function HuntLeadsPage() {
  ...
  function handleExecute(lead: SocialHuntLead) {
    // TODO: PHASE 3 — Wire to on-chain x402 ServiceJob creation via Base
    // For now, open the Farcaster cast in a new tab as a placeholder
    const castUrl = `https://warpcast.com/${lead.targetUsername}`;
      ...
      {!isLoading && !leadsError && filteredLeads.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {filteredLeads.map((lead) => (
            <LeadCard key={lead.id} lead={lead} onExecute={handleExecute} />
          ))}
        </div>
      )}
