...
import { Bot, Rocket, Search, Filter, Loader2 } from 'lucide-react';
import { AgentCard } from '@/components/shared/agent-card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { useAgents, type Agent } from '@/hooks/use-agent';
...
export default function AgentsPage() {
  ...
  const { data: response, isLoading, error } = useAgents(1, 50);
      ...
      {error && !isLoading && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-md">
            {error.message}. Make sure the database is running.
          </p>
        </div>
      )}
